...
const Header = ({ logo }: IHeaderProps) => {
  const brandEl = getItemFromLocalStorage(LOCAL_STORAGE_KEYS.brand) as IBrandData;
  const navigate = useNavigate();
  return (
    <div className={style.header}>
      <div className={style.heder_container}>
        <div className={style.header_flex}>
          <div className={style.header_car}>
            <div
              className={style.headerMain_link}
              onClick={() => {
                navigate(`/`);
              }}
            >
          ...
          <div className={style.header_brand}>
            <img
              className={style.headerBrand_Logo}
              src={!logo ? brandEl.img : logo}
              alt=""
            />
          </div>
        </div>
